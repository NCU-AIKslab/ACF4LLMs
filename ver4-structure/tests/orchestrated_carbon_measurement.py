#!/usr/bin/env python3
"""
Direct orchestrated system test WITH CARBON TRACKING.
Bypasses the orchestrator carbon integration issues to get direct measurements.
"""
...
# Import CodeCarbon for direct emissions tracking
try:
...
def test_orchestrated_system_with_carbon():
    """Test orchestrated system with DIRECT carbon tracking."""
    logger.info("🌱 Starting ORCHESTRATED SYSTEM DIRECT CARBON MEASUREMENT")
    logger.info("=" * 80)

    if not CARBON_AVAILABLE:
        logger.error("❌ CodeCarbon not available!")
        return False, None

    # Initialize carbon tracker
    carbon_tracker = EmissionsTracker(
        project_name="orchestrated_gsm8k_carbon_direct",
        output_dir="./carbon_logs",
        save_to_file=True,
        log_level="WARNING"
    )
    carbon_tracker.start()
    logger.info("✅ Carbon tracking started for orchestrated system")
    ...
    logger.info("🌱 Processing 200 questions with carbon tracking...")

    results = []
    correct_count = 0
    start_time = time.time()

    for i, example in enumerate(tqdm(test_questions, desc="Processing with carbon tracking")):
        try:
    ...
    final_summary = {
        "test_type": "ORCHESTRATED_SYSTEM_DIRECT_CARBON_MEASUREMENT",
        "model_name": model_name,
        "model_type": "orchestrated_quantized_carbon_tracked",
        "status": "COMPLETED",
        "timestamp": time.time(),
        ...
        "carbon_metrics": {
            ...
            "carbon_efficiency_accuracy_per_g_co2": accuracy / ((emissions or 0.001) * 1000)
        },
        "memory_usage": {
    ...
    # Display results
    logger.info("\\n" + "🌱 ORCHESTRATED SYSTEM CARBON RESULTS" + "=" * 50)
    ...
    logger.info(f"🌱 Total CO2: {(emissions or 0.0) * 1000:.3f}g")
    logger.info(f"🌿 CO2/Question: {((emissions or 0.0) * 1000) / len(results):.3f}g")
    logger.info(f"📊 Efficiency: {accuracy / ((emissions or 0.001) * 1000):.3f} accuracy/g CO2")

    # Save results
    results_file = "orchestrated_carbon_measurement_final.json"
    with open(results_file, 'w', encoding='utf-8') as f:
        json.dump(final_summary, f, indent=2, ensure_ascii=False)

    logger.info(f"\\n💾 Results saved to {results_file}")
    logger.info("🌱 ORCHESTRATED CARBON MEASUREMENT COMPLETE!")

    return True, final_summary

if __name__ == "__main__":
    try:
        success, results = test_orchestrated_system_with_carbon()
        if success:
            print("\\n🌱 ORCHESTRATED CARBON MEASUREMENT SUCCESS!")
            print("📊 Direct carbon emission data captured!")
        else:
            print("\\n❌ Carbon measurement failed.")
    except Exception as e:
        print(f"Error: {str(e)}")
        import traceback
        traceback.print_exc()